...
grep -q '^  export HONCHO_ENV_FILE HONCHO_CONFIG_TOML DATA_POSTGRES_DIR DATA_REDIS_DIR HONCHO_PROJECT_NAME$' "$ROOT/lib/shell/common.sh"
...
grep -q '^run_compose "\$COMPOSE_OVERRIDE_FILE" build --pull --no-cache api deriver$' "$ROOT/scripts/shared/honcho-build"
grep -q '^run_compose "\$COMPOSE_OVERRIDE_FILE" up -d database redis api deriver$' "$ROOT/scripts/shared/honcho-start"
grep -q '^run_compose "\$COMPOSE_OVERRIDE_FILE" exec api /bin/sh$' "$ROOT/scripts/shared/honcho-shell"
